import { Content, PT_Sans_Caption } from "next/font/google";
import Image from "next/image";

const ptSanstCaption = PT_Sans_Caption({
    subsets: ['latin'],
    weight: '700',
})


const customerStorys = [
    {
        content: 'Criar minha loja com o site.set foi a melhor decisão para o meu negócio. A plataforma é super intuitiva, e consegui colocar meus produtos à venda em poucos minutos.',
        author: {
            avatar: '/customer-01.png',
            name: 'Annette Bones',
            role: 'CEO na Anne Corp',
        }
    },
    {
        content: 'Transformar minha ideia em uma loja online foi fácil e rápido. Adorei as opções de personalização e a simplicidade para gerenciar os pedidos. Já vejo meus produtos alcançando mais pessoas!',
        author: {
            avatar: '/customer-02.png',
            name: 'Jacob Jones',
            role: 'CEO na JJ Org',
        }
    }
]

export function CustomerStorySection() {
    return (
        <section className="container py-8 md:py-10">
            <div className="flex flex-col items-center gap-12">
                <h2 className={`${ptSanstCaption.className} text-gray-100 text-heading-lg md:text-heading-xl`}>Quem utiliza, aprova!</h2>

                <div className="grid gap-8 md:grid-cols-2">
                    {customerStorys.map((customer) => (
                        <div key={customer.author.name} className="flex flex-col bg-gray-500 gap-6 p-6 md:p-12 rounded-lg">
                            <p className="text-gray-200 text-balance">
                                {customer.content}
                            </p>
                            
                            <div className="flex items-center gap-3">
                                <div className="relative h-10 w-10 overflow-hidden rounded-lg">
                                    <Image 
                                        src={customer.author.avatar}
                                        alt={customer.author.name}
                                        fill
                                        className="object-cover"
                                    />
                                </div>
                                <div className="flex flex-col">
                                    <strong className="text-gray-200 text-sm">{customer.author.name}</strong>
                                    <span className="text-gray-300 text-xs">{customer.author.role}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    )
}